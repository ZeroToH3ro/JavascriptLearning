...
const getArticles = asyncHandler(async (req, res, next) => {
    ...
    if(favorited){
        ...
        articles.count = await user.countFavovites();
    } else {
...
const deleteArticle = asyncHandler(async(req, res, next) => {
    ...
    if(!article) {
        next(new ErrorResponse("Article not fount", 400))
    }

    if(article.authorId === loggedUser.authorId){
...
const articlesFeed = asyncHandler(async(req, res, next) => {
    const {loggedUser} = req;
    ...
    res.status(200).json({articles: articles.rows, articlesCount: articles.count})
...
const addFavoriteArticle = asyncHandler(async(req, res, next) => {
    const {loggedUser} = req;
    const {slug} = req.params;
...
const deleteFavoriteArticle = asyncHandler(async() => {
    const {loggedUser} = req;
    const {slug} = req.params;
