...
const fieldValidation = (field, next) => {
    if(!field) return next(new ErrorResponse(`Missing fields`, 400));
}
...
const getComments = asyncHandler(async(req, res, next) => {
    ...
    const comment = await article.getComments({
        include: [
            {
                model: User,
                as: "author",
        ...
        include: [
            {
                model: User,
                as: "author",
                attributes: ['bio', 'username', "image"]
            }
        ]
    });

    if(!article) return next(new ErrorResponse("Article not fount", 404));
